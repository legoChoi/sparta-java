...
public class Main {
    public static void main(String[] args) {
        ...
        // 책 제목에 "여행"이 들어가는 책 제목 조회
        System.out.println("=================책 제목에 \"여행\"이 포함된 책=================");

        bookList.stream()
                .filter(book -> book.getBookName().contains("여행"))
                .forEach(book -> System.out.println(book.getBookName()));

        // 책 가격이 16200원 이하인 책 제목 조회
        System.out.println("\n=================책 가격이 16200원 이하인 책=================");
        ...
        double max = bookList.stream()
                .mapToDouble(Book::getPrice)
                .max().getAsDouble();

        System.out.println(max + "원");

        // 카테고리가 IT인 책들의 가격의 합 조회
        System.out.println("\n=================카테고리가 IT인 책들의 가격의 합=================");

        double sum = bookList.stream()
                .filter(book -> book.getCategory().contains("IT"))
                .mapToDouble(Book::getPrice)
                .sum();
        ...
        bookList.stream()
                ...
                .forEach(book ->
                        System.out.println("할인 된 책: " + book.getBookName()
                                + "\n할인 된 가격: " + book.getPrice() + "\n"));
